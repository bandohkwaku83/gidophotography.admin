...
export function ClientGalleryApp({ token }: { token: string }) {
  ...
  const [gridLayout, setGridLayout] = useState<GridLayout>("uniform");
  ...
  async function submitSelections() {
    ...
    try {
      ...
      showToast("Selections submitted.", "success");
    } catch (e) {
        ...
        <div className="relative overflow-hidden border-b border-zinc-200/90 bg-gradient-to-br from-indigo-50 via-white to-zinc-50 dark:border-zinc-800 dark:from-zinc-900 dark:via-zinc-950 dark:to-black">
          ...
          {gallery.coverImageUrl ? (
            <div className="relative left-1/2 w-screen max-w-[100vw] -translate-x-1/2 overflow-hidden">
              <div className="relative min-h-[200px] h-[min(42vw,300px)] sm:min-h-[220px] sm:h-[min(34vw,320px)] lg:h-[min(28vw,360px)]">
                ...
                />
                <div
                  className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent dark:from-black/[0.92]"
                  aria-hidden
                />
                <div className="absolute inset-x-0 bottom-0 top-0 flex flex-col justify-end">
                  <div className="mx-auto w-full max-w-6xl px-4 pb-4 pt-6 sm:px-6 sm:pb-5 lg:px-8">
                    <h1 className="max-w-3xl text-balance text-2xl font-bold leading-tight tracking-tight text-white drop-shadow-sm sm:text-3xl lg:text-4xl">
                      {displayTitle}
                    </h1>
                    <p className="mt-1.5 max-w-2xl text-xs leading-snug text-white/85 sm:text-sm">
                      Tap a photo to select it — tabs and layout live below.
                    </p>

                    <div className="mt-3 rounded-xl border border-white/20 bg-black/50 px-3 py-2.5 shadow-lg backdrop-blur-md sm:px-4 sm:py-3">
                      <div className="flex flex-col gap-2.5 sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-5 sm:gap-y-1">
                        <div className="flex min-w-0 items-center gap-2.5">
                          <div
                            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-brand to-indigo-700 text-xs font-bold text-white shadow-md sm:h-10 sm:w-10 sm:text-sm"
                            aria-hidden
                          >
                        ...
                        {eventDateLabel ? (
                          <>
                            <div
                              className="hidden h-7 w-px shrink-0 bg-white/20 sm:block"
                              aria-hidden
                            />
                            <div className="flex items-center gap-2 text-sm text-white">
                              <CalendarDays className="h-4 w-4 shrink-0 text-white/70" aria-hidden />
                      ...
                      {gallery.description ? (
                        <p className="mt-2 border-t border-white/10 pt-2 text-xs leading-snug text-white/80">
                          {gallery.description}
                        </p>
                      ) : null}
                      {gallery.selectionLocked && !gallery.selectionSubmitted ? (
                        <p className="mt-2 rounded-lg border border-amber-400/30 bg-amber-500/15 px-2.5 py-1.5 text-xs text-amber-50">
      ...
      {gallery.selectionSubmitted && (
        <div className="mx-auto max-w-3xl px-4 py-3">
          <p className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-100">
            Your selections have been submitted. Thank you!
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">
              You chose {selectedCount} image{selectedCount === 1 ? "" : "s"}. This sends your picks
